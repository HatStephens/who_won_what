class GroupUser < ActiveRecord::Base
  ...
  def total_matches_won
    fixture_group_users.map{ |fgu| fgu.amount_of_wins }.sum
  end

  def total_matches_lost
    fixture_group_users.map{ |fgu| fgu.amount_of_losses }.sum
  end

  def total_matches_drawn
    fixture_group_users.map{ |fgu| fgu.amount_of_draws }.sum
  end

  def total_big_wins
    fixture_group_users.map{ |fgu| fgu.amount_of_big_wins }.sum
  end
  ...
  def total_bore_draws
    fixture_group_users.map{ |fgu| fgu.amount_of_bore_draws }.sum
  end

  def total_clean_sheets
    fixture_group_users.map{ |fgu| fgu.amount_of_clean_sheets }.sum
  end
